...
if(type == "mm") {
  ...
  Traw <- nrow(df)
  Yraw <- df
  VAR_data <- data.frame(subset_data[c(5,9,10,2)])
}

if(type == "mm_medium") {
  df <- data.frame(subset_data[c(1,5,9,10,3,2,7,6,4)]) 
  df <- na.omit(df)
  Traw <- nrow(df)
  Yraw <- df
  VAR_data <- data.frame(subset_data[c(5,9,10,3,2,7,6,4)])
}
...
if(type == "sd"){
  ...
  Traw <- nrow (df)
  Yraw <- df
  VAR_data <- data.frame(subset_data[c(5,9,11,2)])
}
...
irf2 <- vars::irf(var_mm, impulse = "exports_log", response = "p_wheat_log_diff", n.ahead = 12, boot = TRUE, nboot = nboot, ci = 0.95, boot.type = "rdwb")
...
if (type=="mm_medium") {
  ...
  abline(v = seq(1, by = 2), col = "lightgrey", lty = 2)
  axis(1, at = seq(1, by = 2), labels = seq(0, by = 2))
  plot(irf2, ylim = c(-0.03, 0.02), main = "Shock exports_log on p_wheat_log_diff", ylab = "")
  abline(v = seq(1, by = 2), col = "lightgrey", lty = 2)
  axis(1, at = seq(1, by = 2), labels = seq(0, by = 2))
  plot(irf3, ylim = c(-0.03, 0.08), main = "Shock netlong_mm on p_wheat_log_diff", ylab = "")
  abline(v = seq(1, by = 2), col = "lightgrey", lty = 2)
...
if (type=="sd") {
  df_var_sd<-(data.frame(ind_prod_log_diff,exports_log,netlong_sd,p_wheat_log_diff))
  is.na(df_var_sd)
  var_sd<-VAR(na.omit(df_var_sd), lag.max = 1, ic = "SC", type = "const")
  summary(var_sd)
...
if (type=="sd") {
  ...
  irf2 <- vars::irf(var_sd, impulse = "exports_log", response = "p_wheat_log_diff", n.ahead = 12, boot = TRUE, nboot = nboot, ci = 0.95, boot.type = "rdwb")
